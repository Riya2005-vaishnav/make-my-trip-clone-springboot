...
const BookFlightPage = () => {
    ...
    const fetchPricing = async () => {
      try {
        const [price, history] = await Promise.all([
          getFlightDynamicPrice(id),
          getPriceHistory("FLIGHT", id),
        ]);
        setPriceInfo(price);
        setPriceHistory(history);
      } catch (error) {
        console.error("Error fetching dynamic flight price:", error);
      }
    };
  ...
  const activeFreeze =
    priceFreeze && new Date(priceFreeze.expiresAt).getTime() > Date.now()
      ? priceFreeze
      : null;
  const unitPrice = activeFreeze?.lockedUnitPrice || priceInfo?.dynamicPrice || flight?.price;
  ...
  const handlePriceFreeze = async () => {
    if (!user?.id) {
      setPriceMessage("Please log in to freeze this fare.");
      return;
    }
    try {
      const freeze = await freezeFlightPrice(user.id, flight.id, quantity);
      setPriceFreeze(freeze);
      setPriceMessage(
        `Price frozen at Rs. ${freeze.lockedUnitPrice.toLocaleString()} per ticket until ${new Date(
          freeze.expiresAt
        ).toLocaleTimeString()}`
      );
    } catch (error) {
    ...
    <div className="border rounded-lg p-4 mb-4 bg-blue-50">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-600">
            {priceInfo?.reason || "Fetching the latest fare..."}
          </p>
        </div>
        <div className="text-right">
          <div className="text-xs text-gray-500">Live fare</div>
      ...
      {activeFreeze && (
        <p className="mt-3 text-sm font-medium text-green-700">
          Frozen fare active until {new Date(activeFreeze.expiresAt).toLocaleTimeString()}
        </p>
      )}
