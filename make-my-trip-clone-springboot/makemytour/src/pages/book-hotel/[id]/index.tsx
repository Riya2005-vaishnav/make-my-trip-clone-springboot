...
const BookHotelPage = () => {
    ...
    const fetchPricing = async () => {
      try {
        const [price, history] = await Promise.all([
          getHotelDynamicPrice(id),
          getPriceHistory("HOTEL", id),
        ]);
        setPriceInfo(price);
        setPriceHistory(history);
      } catch (error) {
        console.error("Error fetching dynamic hotel price:", error);
      }
    };

    fetchPricing();
    const interval = window.setInterval(fetchPricing, 15000);
  ...
  const activeFreeze =
    priceFreeze && new Date(priceFreeze.expiresAt).getTime() > Date.now()
      ? priceFreeze
      : null;
  const unitPrice = activeFreeze?.lockedUnitPrice || priceInfo?.dynamicPrice || hotel?.pricePerNight;
  ...
  const handlePriceFreeze = async () => {
    if (!user?.id) {
      setPriceMessage("Please log in to freeze this room price.");
      return;
    }
    try {
      const freeze = await freezeHotelPrice(user.id, hotel.id, quantity);
      setPriceFreeze(freeze);
      setPriceMessage(
        `Price frozen at Rs. ${freeze.lockedUnitPrice.toLocaleString()} per room until ${new Date(
          freeze.expiresAt
        ).toLocaleTimeString()}`
      );
    } catch (error) {
      console.error(error);
      setPriceMessage("Could not freeze this price. Please try again.");
    }
  };
  const HotelContent = () => (
    ...
    <div className="border rounded-lg p-4 mb-6 bg-blue-50">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-600">
            {priceInfo?.reason || "Fetching the latest room price..."}
          </p>
        </div>
        <div className="text-right">
          <div className="text-xs text-gray-500">Live room</div>
          <div className="text-xl font-bold text-blue-700">
            Rs. {(priceInfo?.dynamicPrice || 0).toLocaleString()}
          </div>
        </div>
      </div>
      ...
      {activeFreeze && (
        <p className="mt-3 text-sm font-medium text-green-700">
          Frozen price active until {new Date(activeFreeze.expiresAt).toLocaleTimeString()}
        </p>
      )}
      {priceMessage && <p className="mt-3 text-sm text-gray-700">{priceMessage}</p>}
      <Button type="button" className="w-full mt-3" onClick={onFreeze}>
        Freeze this price for 15 minutes
      </Button>
    </div>
  );
}

function PriceHistoryChart({ history }: any) {
  const points = history || [];
  if (points.length === 0) {
    return (
      <div className="mt-4 h-24 rounded bg-white text-sm text-gray-500 flex items-center justify-center">
        No price history yet
      </div>
    );
  }

  const values = points.map((point: any) => point.dynamicPrice);
  ...
  const path = points
    .map((point: any, index: number) => {
      ...
      const y = 90 - ((point.dynamicPrice - min) / range) * 70;
      return `${index === 0 ? "M" : "L"} ${x} ${y}`;
    })
    .join(" ");

  return (
    <svg viewBox="0 0 300 100" className="mt-4 w-full h-28 bg-white rounded">
      <path d={path} fill="none" stroke="#2563eb" strokeWidth="3" />
      <text x="8" y="18" className="fill-gray-500 text-[10px]">
        Rs. {max.toLocaleString()}
      </text>
      <text x="8" y="94" className="fill-gray-500 text-[10px]">
